...
Global $GTFOcheck, $hGUI_DONATE, $hGUI_GTFOMode, $grpGtfo, $cmbGtfo, $btnGtfoStart, $btnGtfoPause, $btnGtfoStop, $chkMassDonate, $chkClanOpen, $chkWaitForTroops, $chkGtfoStats
...
Func GUIControl_WM_COMMAND($hWind, $iMsg, $wParam, $lParam)
	...
	Switch $nID
		...
		Case $btnTestConfigRead
			readConfig()
		case $btnTestEmulatorBar
			AndroidBotStartEvent()
		Case $btnTestAttackNowLB
			btnTestAttackNowLB()
		Case $btnTestConfigApply
			applyConfig()
		case $btnTestCheckArmySpellCastel
			btnTestCheckArmySpellCastel()
		Case $btnGtfoStart
			GTFOStart()
		Case $btnGtfoPause
			GTFOPause()
		Case $btnGtfoStop
			GTFOStop()
        Case $lstGtfoChatTemplates
            Switch $nNotifyCode
                Case $LBN_DBLCLK
                    GtfoTemplate_DoubleClick()
			EndSwitch
	EndSwitch
...
$tabidx = GUICtrlRead($hGUI_DONATE_TAB)
$tabgtx = _GUICtrlTab_GetItemRect($hGUI_DONATE_TAB, 1)

	Select
		case $tabidx = 0
			GUISetState(@SW_HIDE, $hGUI_GTFOMode)
			GUICtrlSetPos($GTFOcheck, $tabgtx[2] - 15, $tabgtx[3] - 15)
		case $tabidx = 1
			ConsoleWrite( $tabgtx[2] - 15 & "   " & $tabgtx[3] - 15 )
			GUICtrlSetPos($GTFOcheck, $tabgtx[2] - 15, $tabgtx[3] - 17)
			If GUICtrlRead($GTFOcheck) = $GUI_CHECKED Then
				GUISetState(@SW_SHOW, $hGUI_GTFOMode)
				GUICtrlSetState($lblGTFOdisabled, $GUI_HIDE)
			Else
				GUISetState(@SW_HIDE, $hGUI_GTFOMode)
				GUICtrlSetState($lblGTFOdisabled, $GUI_SHOW)
			EndIf
			;GUISetState(@SW_HIDE, $hGUI_UPGRADE)
			;GUISetState(@SW_SHOWNOACTIVATE, $hGUI_DONATE)
			;GUISetState(@SW_HIDE, $hGUI_NOTIFY)
...
Func GTFOcheck()
	If $iBotLaunchTime > 0 Then _GUICtrlTab_SetCurFocus($hGUI_DONATE_TAB, 1)
	If GUICtrlRead($GTFOcheck) = $GUI_CHECKED Then
		GUISetState(@SW_SHOW, $hGUI_GTFOMode)
		GUICtrlSetState($lblGTFOdisabled, $GUI_HIDE)
		_GUICtrlEdit_SetText($txtLog, _PadStringCenter(" GTFO LOG ", 70, "="))
	Else
		GUISetState(@SW_HIDE, $hGUI_GTFOMode)
		GUICtrlSetState($lblGTFOdisabled, $GUI_SHOW)
...
If $devmode = 1 Then
	...
	GUICtrlSetState($chkDebugSX, $GUI_SHOW + $GUI_ENABLE)
