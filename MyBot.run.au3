...
Global $sModversion = "v0.8.1" ;<== Just Change This to Version Number
...
If GUICtrlRead($GTFOcheck) = $GUI_CHECKED Then
	...
	_GUICtrlTab_ClickTab($tabmain, 3)
	_GUICtrlTab_ClickTab($hGUI_BOT_TAB, 4)
	_GUICtrlTab_ClickTab($hGUI_STATS_TAB, 4)
	_GUICtrlTab_ClickTab($tabmain, 1)
	_GUICtrlTab_ClickTab($hGUI_VILLAGE_TAB, 1)
	_GUICtrlTab_ClickTab($hGUI_DONATE_TAB, 1)
...
Func runBot() ;Bot that runs everything in order
	...
	While 1
		...
		If $Is_ClientSyncError = False And $Is_SearchLimit = False And ($Quickattack = False) Then
			...
			checkMainScreen(False)
			If $Restart = True Then ContinueLoop
			Local $aRndFuncList = ['Collect', 'CheckTombs', 'ReArm', 'CleanYard', 'GTFOKICK']
			While 1
